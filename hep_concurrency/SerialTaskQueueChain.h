...
namespace hep {
  namespace concurrency {
    ...
    public:
      ...
      SerialTaskQueueChain&
      operator=(SerialTaskQueueChain&& iOld)
      {
        m_queues = std::move(iOld.m_queues);
      ...
      unsigned long
      outstandingTasks() const
      {
        return m_outstandingTasks;
      }
      std::size_t
      numberOfQueues() const
      {
    ...
    template <typename T>
    void
    SerialTaskQueueChain::push(const T& iAction)
    {
      ++m_outstandingTasks;
      if (m_queues.size() == 1) {
        ...
        m_queues[0]->push(
          [this, iAction]() { this->passDownChain(1, iAction); });
      }
    }

    template <typename T>
    void
    SerialTaskQueueChain::pushAndWait(const T& iAction)
    {
    ...
    template <typename T>
    void
    SerialTaskQueueChain::passDownChain(unsigned int iQueueIndex,
                                        const T& iAction)
    {
    ...
    template <typename T>
    void
    SerialTaskQueueChain::actionToRun(const T& iAction)
    {
