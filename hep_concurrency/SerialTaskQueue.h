...
/**\class SerialTaskQueue SerialTaskQueue.h "hep_concurrency/SerialTaskQueue.h"
 ...
 Usage:
    A SerialTaskQueue is used to provide thread-safe access to a resource. You
create a SerialTaskQueue for the resource. When every you need to perform an
operation on the resource, you push a 'task' that does that operation onto the
queue. The queue then makes sure to run one and only one task at a time. This
guarantees serial access to the resource and therefore thread-safety.

    The 'tasks' managed by the SerialTaskQueue are just functor objects who
which take no arguments and return no values. The simplest way to create a task
is to use a C++11 lambda.
...
namespace hep {
  namespace concurrency {
    ...
    public:
       ...
       * \param[in] iAction Must be a functor that takes no arguments and return
       * no values.
       */
      template <typename T>
       ...
       * finish. In that way the core is not idled while waiting. \param[in]
       * iAction Must be a functor that takes no arguments and return no values.
       */
      template <typename T>
      void pushAndWait(const T& iAction);

      /// asynchronously pushes functor iAction into queue and finds next task
      /// to execute
      /**
       * This function is useful if you are accessing the SerialTaskQueue for
       * the execute() method of a TBB task and want to efficiently schedule the
       * next task from the queue. In that case you can take the return value
       * and return it directly from your execute() method. The function will
       * return immediately and not wait for iAction to run. \param[in] iAction
       * Must be a functor that takes no arguments and return no values. \return
       * Returns either the next task that the user must schedule with TBB or a
       * nullptr.
       */
      template <typename T>
    ...
    private:
      ...
      private:
        void
        setQueue(SerialTaskQueue* iQueue)
        {
          m_queue = iQueue;
        }
    ...
    template <typename T>
    void
    SerialTaskQueue::push(const T& iAction)
    {
      QueuedTask<T>* pTask{new (tbb::task::allocate_root())
                             QueuedTask<T>{iAction}};
      pTask->setQueue(this);
      pushTask(pTask);
    }
    ...
    template <typename T>
    void
    SerialTaskQueue::pushAndWait(const T& iAction)
    {
      tbb::empty_task* waitTask =
        new (tbb::task::allocate_root()) tbb::empty_task;
      waitTask->set_ref_count(2);
      QueuedTask<T>* pTask{new (waitTask->allocate_child())
                             QueuedTask<T>{iAction}};
      pTask->setQueue(this);
    ...
    template <typename T>
    tbb::task*
    SerialTaskQueue::pushAndGetNextTaskToRun(const T& iAction)
    {
      QueuedTask<T>* pTask{new (tbb::task::allocate_root())
                             QueuedTask<T>{iAction}};
      pTask->setQueue(this);
    ...
    inline tbb::task*
    SerialTaskQueue::TaskBase::finishedTask()
    {
